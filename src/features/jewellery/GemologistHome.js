...
const GemologistHome = ({ navigation }) => {
  ...
  const [selectedTab, setSelectedTab] = useState("Gemologists");
  ...
  const gemologistId = decodedPayload.id;
  const user = useSelector((state) => state.user.user);
  const fromGemologistId = useSelector((state) => state.user.user.roleData._id);
  ...
  const handleVendorConnect = async (toVendorId) => {
    try {
      ...
      console.log("GemologistId: ", fromGemologistId);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userVendors = data.filter(
          (vendor) =>
            vendor.connectedGemologists &&
            vendor.connectedGemologists.includes(userId)
        );
  ...
  const fetchGemologists = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const filteredGemologists = data.filter(
          (gemologist) => gemologist._id !== userId
        );
        console.log("Filtered Designers Data: ", filteredGemologists);
        setGemologists(filteredGemologists);
      } else {
        ...
        <TouchableOpacity style={styles.modalBackground} onPress={toggleModal}>
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <TouchableOpacity
                onPress={() => {
                  handleVendorConnect(selectedVendorId, gemologistId, userType);
                }}
                style={styles.option}
              >
                <Ionicons
      ...
      <View>
        {selectedTab === "Gemologists" && (
          ...
          >
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} scrollEnabled={jeweleryTools.length > 2}>
              <View style={{ flexDirection: "row" }}>
