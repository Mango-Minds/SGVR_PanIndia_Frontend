...
const fetchConnectionRequests = async (userId) => {
  const response = await apiClient.get(
    `/matrimony/connection/requests/${userId}`
  );
...
const acceptConnectionRequest = async (requestId) => {
  const response = await apiClient.post(
    `/matrimony/connection/accept-request/${requestId}`,
    {
      action: "accepted",
    }
  );
...
const rejectConnectionRequest = async (requestId) => {
  const response = await apiClient.post(
    `/matrimony/connection/reject-request/${requestId}`,
    {
      action: "rejected",
    }
  );
...
const sendConnectionRequest = async ({ senderId, receiverId, createdBy }) => {
  const response = await apiClient.post("/matrimony/connection/send-request", {
    senderId,
    receiverId,
    createdBy,
  });
...
//   const connectToChat = async (owner_id, business_id, vendorId, navigation, t) => {

//   if (owner_id === business_id) {
//     alert(t("chat.sameIdError"));
//     return;
//   }
...
//           if (room_with_user) {
//             Alert.alert(t("chat.successTitle"), t("chat.roomCreated"), [
//               {
//                 text: t("chat.ok"),
//                 onPress: () => {
...
//             ]);
//           } else {
//             Alert.alert(t("chat.noMatchingRoom"));
//           }
//         } else {
//           Alert.alert(t("chat.noRoomsFound"));
//         }
//       } else {
//         Alert.alert(t("chat.errorFetchingRooms"));
//       }
//     } else {
//       Alert.alert(t("chat.errorCreatingRoom"));
//     }
//   } catch (error) {
//     console.error("Error:", error);
//     Alert.alert(t("chat.unexpectedError"));
//   }
// };
const connectToChat = async (
  ...
  try {
    ...
    const createRoomUrl = `/chat/room/`;
    console.log("POST URL:", createRoomUrl);
    ...
    if (response.status === 200) {
      const getRoomsUrl = `/chat/rooms/`;
      console.log("GET URL:", getRoomsUrl);

      const roomResponse = await apiClient.get(getRoomsUrl, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        if (roomData?.rooms?.length > 0) {
          ...
          const room_with_user = roomData.rooms.find(
            (room) =>
              ...
              room.participants[0].id === vendorId
          );
